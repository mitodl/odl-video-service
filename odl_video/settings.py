...
AWS_S3_UPLOAD_MULTIPART_THRESHOLD_MB = get_int(
    "AWS_S3_UPLOAD_MULTIPART_THRESHOLD_MB", 32
)
AWS_S3_UPLOAD_MULTIPART_CHUNKSIZE_MB = get_int(
    "AWS_S3_UPLOAD_MULTIPART_CHUNKSIZE_MB", 32
)
...
ET_PRESET_IDS = get_string(
    "ET_PRESET_IDS", "1351620000001-200010,1351620000001-200020,1351620000001-200050"
).split(",")

if ET_PRESET_IDS == [""]:  # This may happen if `ET_PRESET_IDS=` is in .env file.
    raise ImproperlyConfigured(
        "ET_PRESET_IDS cannot be blank, please check your settings & environment"
    )
...
FEATURES = {
    key[len(ODL_VIDEO_FEATURES_PREFIX) :]: get_any(key, None)
    for key in get_all_config_keys()
    if key.startswith(ODL_VIDEO_FEATURES_PREFIX)
}
