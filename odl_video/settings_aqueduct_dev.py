"""Opt-in django-aqueduct settings module for local dev, backed by Vault (OIDC).

Select this module by setting
`DJANGO_SETTINGS_MODULE=odl_video.settings_aqueduct_dev`. Missing values are
filled in from Vault via an interactive OIDC login instead of requiring a
local `.env` file. `odl_video.settings` (the default) is untouched.

See `odl_video/aqueduct_settings.py` for the underlying Pydantic model
(`DevAqueductSettings`) and its `VaultSettingsSource` wiring.
"""
...
# while building DevAqueductSettings are still captured.
init_sentry(
