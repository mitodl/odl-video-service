...
class DropboxToS3Transfer:
    ...
    def __init__(
        ...
        *,
        ...
        # transport live in the caller (cloudsync.dropbox_api); retries live here.
    ...
    def _fetch_range(self, start, end):
        ...
        Each attempt calls ``range_fetcher`` afresh, which re-authenticates the Dropbox
        download, so an expired token or signed CDN URL is re-resolved for free. The last
        error is raised once the attempt budget is exhausted (e.g. a permanently revoked link).
        """
        last_error = None
