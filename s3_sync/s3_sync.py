...
parser.add_argument('-i', dest="settings_file", required=True, help='ini file containing configs', metavar='FILE')
...
def verify_aws_cli_installed(aws_cli_binary):
	...
	if os.path.exists(aws_cli_binary):
		...
		logger.exception("Could not find AWS CLI executable:")
...
def sync_local_to_s3(local_video_records_done_folder, s3_bucket_name):
	...
	try:
		...
		notify_slack_channel("Sync failed: " + computer_name + "*" + "\n" + "`" + err + "`")
...
def main():
	...
	verify_aws_cli_installed(config['Paths']['aws_cli_binary'])
