"""Management command to update video to edX via API call"""  # noqa: INP001
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--video-key",
            type=str,
            help="The UUID key of the Video that you want to update on its configured edX endpoint",  # noqa: E501
        )
        parser.add_argument(
            "--chunk-size",
            type=int,
        ...
        parser.add_argument(
            "--all",
            action="store_true",
            help="Update all of videos to their configured edX endpoints (this may take a long time)",  # noqa: E501
        )

    def handle(self, *args, **options):  # noqa: ARG002
        if not options["video_key"] and not options["all"]:
            raise CommandError("Please provide either --video-id or --all")  # noqa: EM101, TRY003

        if options["video_key"]:
            video_key = options["video_key"]
            video = Video.objects.filter(key=video_key).first()
            if video is None:
                raise CommandError("This video key doesn't exist")  # noqa: EM101, TRY003
            ...
            if "failed" in list(result["kwargs"]["tasks"][0].values()):
                ...
                raise CommandError("Failed to update some video(s) to edX")  # noqa: EM101, TRY003

            total_seconds = (now_in_utc() - start).total_seconds()
            self.stdout.write(
                f"Updating video(s) to edX finished, took {total_seconds} seconds.....\n"  # noqa: E501
            )
