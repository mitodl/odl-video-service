...
class Command(BaseCommand):
    ...
    help = "Attempts to add an HLS video to edX via API call"
    ...
    def handle(self, *args, **options):
        ...
        if options["video_file_id"]:
            video_files = [VideoFile.objects.get(pk=options["video_file_id"])]
        else:
        ...
        if not video_files:
            raise CommandError("No HLS-encoded VideoFiles found that match the given parameters")

        self.stdout.write(
            "Attempting to post video(s) to edX..."
        )
        for video_file in video_files:
            resp = post_hls_to_edx(video_file)
            if not resp.ok:
                ...
                self.stdout.write(self.style.SUCCESS(
                    "Video successfully added to edX (VideoFile: {})".format(video_file.pk)
                ))
