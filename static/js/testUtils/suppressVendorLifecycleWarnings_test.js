...
describe("suppressVendorLifecycleWarnings", () => {
  ...
  describe("family A: deprecated lifecycles (console.warn)", () => {
    ...
    describe("passes through everything else", () => {
      ...
      // Known sets are PER-LIFECYCLE: KNOWN_NAMES_BY_LIFECYCLE keys one Set
      // per lifecycle, each built from the rows matching that lifecycle only.
      // Drop that filter and the sets collapse into one global union, at
      // which point a name excused for one lifecycle is silently excused for
      // every other one too -- which is exactly the class of change this
      ...
      // when rmwc did, so the case is REBASED here rather than deleted with
      // it -- deleting it would have left the lifecycle filter with no test
      // at all. MemoryRouter is the replacement because it is a known
      ...
      it("a name that is known for a different lifecycle", () => {
        warnPassesThrough(CWRP, "MemoryRouter")
      })
   ...
   *
   * Note that afterEach's remaining two lines are load-bearing and must stay:
   * restore() only rewinds as far as the recorders installed in beforeEach, so
   * without them those recorders -- and their by-then-stale
   * `passedThroughWarn` / `passedThroughError` arrays -- would leak into every
   * test file mocha loads after this one. Two lines rather than one since
   * Phase R3 Task 4, because console.error is now recorded alongside
   * console.warn.
   */
  it("stops suppressing once restore() has run", () => {
    warnSwallows(CWM, KNOWN_CWM_GROUP)
    restore()
    warnPassesThrough(CWM, KNOWN_CWM_GROUP)
  })
})
