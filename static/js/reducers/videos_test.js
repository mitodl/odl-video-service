...
describe('videos endpoint', () => {
  ...
  it("should have some initial state", () => {
    assert.deepEqual(store.getState().videos, { ...INITIAL_STATE, data: new Map() });
  });
  ...
  it('should patch videos', async () => {
    ...
    await dispatchThen(
      actions.videos.patch(video.key, payload),
      [
        actions.videos.patch.requestType,
        actions.videos.patch.successType,
      ]
    );
    assert.deepEqual(store.getState().videos.data.get(video.key), videoWithDifferentTitle);
