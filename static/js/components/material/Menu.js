...
export default class Menu extends React.Component {
  ...
  render() {
    ...
    return <div className="mdc-menu-anchor">
      <a className="material-icons" onClick={showMenu}>more_vert</a>
      <div className="mdc-simple-menu" tabIndex="-1" ref={div => this.menuRoot = div}>
        <ul className="mdc-simple-menu__items mdc-list" role="menu" aria-hidden="true">
          {
            menuItems.map(item => (
              <li key={`${item.label}_item`} className="mdc-list-item" role="menuitem" onClick={item.action}>
                {item.label}
              </li>
            ), this)
          }
