...
describe("Menu", () => {
  ...
  // the way VideoPlayer_test.js stubs `libVideo.videojs`). This is an
  // inference, not a literal restatement, flagged per the dossier for a
  // second pair of eyes: capturing the `.mdc-menu` DOM node reference
  // before the prop change and asserting it is the *same* node after
  // rerender proves React did not remount the subtree, which proves
  // componentDidMount (and therefore `new MDCMenu()`) did not re-run.
  // Paired with asserting the open-accessor setter was never called (a
  // more direct, spy-based substitute for the original's
  ...
  it("does not touch the MDC menu when an unrelated prop changes", () => {
    const openSpy = sandbox.spy(MDCMenu.prototype, "open", ["set"])
    const { container, rerender } = renderMenu({ open: false })
    const menuEl = container.querySelector(".mdc-menu")
    ...
    assert.strictEqual(container.querySelector(".mdc-menu"), menuEl)
