...
export default class AnalyticsInfoTable extends React.Component<*, void> {
          ...
          <tr>
            {
              columnSpecs.map((columnSpec) => {
                return (
                  <th key={columnSpec.key} className={columnSpec.className}
          ...
          <tr>
            {
              columnSpecs.map((columnSpec) => {
                return (
                  <td key={columnSpec.key} className={columnSpec.className}
  ...
  generateColumnSpecs (opts:{ analyticsData: VideoAnalyticsData, time: number }) {
    ...
    if (analyticsData.is_multichannel) {
      columnSpecs = [
        ...
        ...(analyticsData.channels.map((channel) => {
          ...
          const percent = (
            (totalViews === 0) ? 0 : ((numViews / totalViews) * 100)
          )
          return {
            key:       channel,
            ...
            valueEl:   (
              <span>{numViews} ({percent.toFixed(1)}%)</span>
            ),
          }
        }))
      ]
